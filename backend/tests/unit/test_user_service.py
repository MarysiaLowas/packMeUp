...
TEST_HASHED_PASSWORD = bcrypt.hashpw(
    "StrongPass123".encode(), bcrypt.gensalt()
).decode()
...
class TestUserService:
    ...
    async def test_create_user(self, user_exists):
        ...
        if user_exists:
            # Case when user already exists - create raises IntegrityError
            ...
            with patch("app.models.User.create", new_callable=AsyncMock) as mock_create:
                ...
                with pytest.raises(HTTPException) as exc_info:
                    await UserService.create_user(
                        TEST_EMAIL, TEST_PASSWORD, TEST_FIRST_NAME
                    )
                assert exc_info.value.status_code == 400
                assert "already registered" in exc_info.value.detail
        else:
            # Case when user does not exist - create returns user
            with patch("app.models.User.create", new_callable=AsyncMock) as mock_create:
                ...
                assert bcrypt.checkpw(
                    TEST_PASSWORD.encode(), create_args["hashed_password"].encode()
                )

    @pytest.mark.asyncio
    @pytest.mark.parametrize("password_valid", [True, False])
    async def test_authenticate_user(self, mock_user, password_valid):
        ...
        with patch(
            ...
            if password_valid:
                ...
                with patch(
                    "app.services.user_service.UserService.verify_password",
                    return_value=True,
                ):
                    # Act
    ...
    async def test_create_session(self):
        ...
        # Arrange
        with patch(
            "app.models.ActiveSession.delete", new_callable=AsyncMock
        ) as mock_delete:
            mock_delete.return_value = None

            # Mock ActiveSession.create for the new session
            with patch(
                "app.models.ActiveSession.create", new_callable=AsyncMock
            ) as mock_create:
    ...
    async def test_get_active_session(self, mock_session):
        ...
        # Arrange
        with patch(
            "app.models.ActiveSession.select_one", new_callable=AsyncMock
        ) as mock_select_one:
