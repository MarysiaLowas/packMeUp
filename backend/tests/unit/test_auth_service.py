...
from app.services.auth_service import AuthService, Token, TokenData
...
class TestAuthService:
    ...
    async def test_authenticate_success(self, mock_user):
        ...
        # Arrange
        with patch("app.services.user_service.UserService.authenticate_user", 
                  new_callable=AsyncMock) as mock_auth_user:
            mock_auth_user.return_value = mock_user
            
            with patch("app.services.user_service.UserService.create_session", 
    ...
    async def test_authenticate_failure(self):
        ...
        with patch("app.services.user_service.UserService.authenticate_user", 
                  new_callable=AsyncMock) as mock_auth_user:
            mock_auth_user.side_effect = HTTPException(status_code=401, detail="Invalid credentials")
    ...
    async def test_refresh_token_success(self, mock_user):
        ...
        with patch("app.services.user_service.UserService.get_active_session", 
                  new_callable=AsyncMock) as mock_get_session:
            mock_get_session.return_value = mock_session
            
            with patch("app.models.User.get", 
    ...
    async def test_refresh_token_no_session(self):
        ...
        with patch("app.services.user_service.UserService.get_active_session", 
            ...
            with patch("jose.jwt.decode") as mock_jwt_decode:
                ...
                assert (
                    "Invalid refresh token - no active session" in exc_info.value.detail or
                    "Invalid refresh token - unexpected error" in exc_info.value.detail
                )
    ...
    async def test_refresh_token_mismatch(self, mock_user):
        ...
        with patch("app.services.user_service.UserService.get_active_session", 
            ...
            with patch("jose.jwt.decode") as mock_jwt_decode:
                ...
                #
                assert (
                    "token mismatch" in exc_info.value.detail or
                    "Invalid refresh token - token mismatch" in exc_info.value.detail or
                    "Invalid refresh token - unexpected error" in exc_info.value.detail
                )
    ...
    async def test_logout(self):
        """Test user logout"""
        # Arrange
        with patch("app.services.user_service.UserService.delete_session", 
                  new_callable=AsyncMock) as mock_delete_session:
