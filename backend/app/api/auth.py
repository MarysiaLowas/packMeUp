...
class LoginCredentials(BaseModel):
    ...
    model_config = {
        "json_schema_extra": {
            "example": {"username": "user@example.com", "password": "StrongPass123"}
        }
    }
...
def set_auth_cookie(response: Response, token: str):
    """Set auth cookie with proper settings based on environment."""
    response.set_cookie(
        key="refresh_token",
        value=token,
        httponly=True,
...
async def login(response: Response, credentials: LoginCredentials) -> Token:
