    ...
    .array(
      z
        ...
        .refine(
          ...
          {
            message:
              "Musisz podać albo wagę, albo wszystkie wymiary (szerokość, wysokość, głębokość), albo oba",
          },
        ),
    )
    .optional(),
  ...
  const onSubmit = async (data: CreateTripFormShape) => {
    ...
    try {
      ...
      const luggageItems = data.availableLuggage
        ?.filter(
          (item) => item.maxWeight || (item.width && item.height && item.depth),
        )
        .map((item) => {
      ...
      const generatedList =
        await apiClient.post<GeneratePackingListResponseDTO>(
      ...
      setApiError(
        error instanceof Error
          ? error.message
          : "Wystąpił błąd podczas zapisywania podróży",
      );
    } finally {
      setIsLoading(false);
    }
